...
function validDate(value: string) { return /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T00:00:00Z`)) && value <= new Date().toISOString().slice(0,10); }
...
app.post('/api/events', async (c) => {
  const body = await c.req.json<{title?:string;startDate?:string;description?:string}>().catch(() => ({}));
